...
const STATUS_TONE: Record<Task["status"], string> = {
  ...
  doing: "bg-primary-400/10 text-primary-700",
  ...
  if (myTasks.length === 0) {
    return (
      <GlassCard className="p-6 text-center">
        ...
        <p className="mt-2 text-sm text-navy-600">진행 중인 내 업무가 없습니다.</p>
        <Link
          to="/tasks"
  ...
  return (
    <div className="space-y-2">
      {myTasks.map((task) => {
        ...
        return (
          <Link key={task.id} to={`/tasks?task=${task.id}`} className="block">
            <GlassCard className="flex items-center gap-3 p-4 transition hover:bg-sky-50/40">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <p className="truncate font-medium text-navy-900">{task.title}</p>
                  <span
                    className={cn(
