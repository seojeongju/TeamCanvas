...
export function CopyProjectWorkModal({ open, onClose, project }: Props) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title="다른 프로젝트에서 가져오기">
      <form onSubmit={handleSubmit} className="space-y-4">
        <p className="text-sm text-navy-600">
          원본 프로젝트의 업무·마일스톤·체크리스트·연결 일정을 이 프로젝트에 복사합니다. 복사된 항목은
          이후 자유롭게 수정할 수 있습니다.
        </p>
        ...
        <div className="max-h-52 space-y-1 overflow-y-auto rounded-xl border border-sky-100/80 p-2">
          {isLoading ? (
            ...
            <p className="p-3 text-center text-sm text-navy-500">가져올 프로젝트가 없습니다.</p>
          ) : (
            projects.map((p) => {
              const selected = sourceProjectId === p.id;
              return (
                <button
                  key={p.id}
                  type="button"
        ...
        <Button type="submit" fullWidth disabled={copyWork.isPending || !sourceProjectId}>
          {copyWork.isPending ? "복사 중..." : "이 프로젝트로 가져오기"}
        </Button>
      </form>
    </Modal>
  );
