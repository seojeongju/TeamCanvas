...
export function SendProjectWorkModal({ open, onClose, project }: Props) {
  ...
  const canSubmit =
    !!targetProjectId && (selectedTaskIds.size > 0 || includeMilestones) && !copyWork.isPending;
    ...
    <Modal open={open} onClose={onClose} title="다른 프로젝트로 복사">
      <form onSubmit={handleSubmit} className="space-y-4">
        <p className="text-sm text-navy-600">
          적용할 프로젝트를 선택한 뒤, 이 프로젝트에서 내보낼 업무를 체크하세요. 선택한 업무의
          체크리스트·하위 업무도 함께 복사됩니다.
        </p>
        ...
        <ProjectTaskSelectPanel
          ...
          taskSourceProjectId={targetProjectId ? project.id : ""}
          projectSearchPlaceholder="적용할 프로젝트 검색"
          emptyProjectsText="적용할 프로젝트가 없습니다."
        />
        ...
        <Button type="submit" fullWidth disabled={!canSubmit}>
          {copyWork.isPending
            ...
            : selectedTaskIds.size > 0
              ? `선택한 업무 ${selectedTaskIds.size}건 내보내기`
              : "마일스톤만 내보내기"}
        </Button>
      </form>
    </Modal>
